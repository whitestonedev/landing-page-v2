...
export function AuthorCard({ author, variant = 'default' }: AuthorCardProps) {
  ...
  return (
    <Card className={`flex flex-row items-center gap-3 px-4 py-2 shadow-none border border-muted-foreground/10 bg-muted/40 w-full sm:w-auto ${isCompact ? 'text-sm' : ''}`}>
      <img
        src={author.image}
        alt={author.name}
