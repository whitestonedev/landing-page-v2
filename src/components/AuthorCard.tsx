...
export function AuthorCard({ author, variant = "default" }: AuthorCardProps) {
  ...
  return (
    ...
    >
      <Card
        className={`flex flex-row items-center gap-3 px-4 py-2 shadow-none border border-muted-foreground/10 bg-muted/40 w-full sm:w-auto min-h-[90px] ${
          isCompact ? "text-sm" : ""
          ...
          <div className="flex gap-1 mt-1" onClick={(e) => e.stopPropagation()}>
            {author.github && (
              <Button
                variant="ghost"
                size="icon"
                className={`${isCompact ? "h-5 w-5" : "h-6 w-6"} p-0`}
                asChild
              >
                <a
                  href={author.github}
